...
unsafe fn cooley_tukey_radix_2_kernel<M: Modulo>(
    ...
    if offset == 1 && blocks >= 8 {
        ...
        for block in (0..blocks).step_by(8) {
            ...
            let c1 = M32x8::gather(a[top + 1..].as_ptr(), vindex) * rot;
...
unsafe fn cooley_tukey_radix_4_kernel<M: Modulo>(
    ...
    if offset == 1 && blocks >= 8 {
        ...
        for block in (0..blocks).step_by(8) {
            ...
            let rot = M32x8::<M>::load(r.as_ptr());
            let rot2 = rot * rot;
            let rot3 = rot * rot2;
            let (r0, r1, r2, r3) = radix4_inner!(
                M32x8::gather(a[top..].as_ptr(), vindex),
                M32x8::gather(a[top + 1..].as_ptr(), vindex) * rot,
                M32x8::gather(a[top + 2..].as_ptr(), vindex) * rot2,
                M32x8::gather(a[top + 3..].as_ptr(), vindex) * rot3,
                imag
            );
        ...
        for block in (0..blocks).step_by(4) {
            ...
            let (r0, r1, r2, r3) = radix4_inner!(
                M32x8::gather(a[top..].as_ptr(), vindex),
                M32x8::gather(a[top + 2..].as_ptr(), vindex) * rot,
                M32x8::gather(a[top + 4..].as_ptr(), vindex) * rot2,
                M32x8::gather(a[top + 6..].as_ptr(), vindex) * rot3,
                imag
            );
        ...
        for block in (0..blocks).step_by(2) {
            ...
            let next_rot = rot * rate[block.trailing_ones() as usize];
            {
                ...
                let (r0, r1, r2, r3) = radix4_inner!(
                    M32x8::gather(a[top..].as_ptr(), vindex),
                    M32x8::gather(a[top + 4..].as_ptr(), vindex) * rot,
                    M32x8::gather(a[top + 8..].as_ptr(), vindex) * rot2,
                    M32x8::gather(a[top + 12..].as_ptr(), vindex) * rot3,
                    imag
                );
                storeu(
                    a[top..].as_mut_ptr() as _,
        ...
        for block in 1..blocks {
            ...
            rot = rot * M32x8::splat(*rate.get_unchecked(block.trailing_ones() as usize));
        }
    }
}
